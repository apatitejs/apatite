...
let ApatiteQueryBuilder = require('../query/apatite-query-builder')
...
class ApatiteSession {
    ...
    /**
     * Checks for existence of given table name. If the argument onChecked is not provided, an instance of Promise is returned.
     * 
     * @param {String} tableName A string containing the table name.
     * @param {function(Error, Object)} onChecked Optional. A callback function which would be called after the check. The function would be called with two arguments, the first an error object or null, the second an object containing the rows (Array) with table infos if the table exists, else an empty array.
     * @returns {Promise | undefined} Instance of Promise or undefined.
     * 
     */
    existsDBTable(tableName, onChecked) {
        const stmt = this.getSQLScriptCreator().createStatementForTableExistence(tableName)
        return this.basicExecuteDBObjectExistence(stmt, onChecked)
    }

    /**
     * Checks for existence of given column name in the provided table name. If the argument onChecked is not provided, an instance of Promise is returned.
     * 
     * @param {String} tableName A string containing the table name.
     * @param {String} columnName A string containing the table column name.
     * @param {function(Error, Object)} onChecked Optional. A callback function which would be called after the check. The function would be called with two arguments, the first an error object or null, the second an object containing the rows (Array) with table column infos if the column exists, else an empty array.
     * @returns {Promise | undefined} Instance of Promise or undefined.
     * 
     */
    existsDBTableColumn(tableName, columnName, onChecked) {
        const stmt = this.getSQLScriptCreator().createStatementForTableColumnExistence(tableName, columnName)
        return this.basicExecuteDBObjectExistence(stmt, onChecked)
    }

    basicExecuteDBObjectExistence(stmt, onChecked) {
        if (onChecked) {
            this.connection.executeSQLString(stmt.sqlString, stmt.bindings, onChecked, { isApatiteDirectSql: true, resultSet: true })
            return
        }
        return new Promise((resolve, reject) => {
            this.connection.executeSQLString(stmt.sqlString, stmt.bindings, function (err, result) {
                if (err) {
                    reject(err)
                } else {
                    resolve(result)
                }
            }, { isApatiteDirectSql: true, resultSet: true })
        })
    }
     ...
     * 
     * @param {class} model A valid javascript class.
     * @param {Array} expressionArr An array expression containing where clause.
     * @returns {ApatiteQuery} An instance of class ApatiteQuery.
     * 
     */
    newQueryFromArray(model, expressionArr) {
        let queryBuilder = new ApatiteQueryBuilder(expressionArr)
        let newQuery = this.newQuery(model)
        queryBuilder.buildQuery(newQuery)
        return newQuery
    }
