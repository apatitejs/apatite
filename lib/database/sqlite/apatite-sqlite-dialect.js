...
class ApatiteSqliteDialect extends ApatiteDialect {
    ...
    buildSQLForTableExistence(tableName, bindVariables) {
        ...
        return `SELECT * FROM sqlite_master WHERE type = 'table' AND name = ${bindVar.getVariableName()}`
    }

    buildSQLForTableColumnExistence(tableName, columnName, bindVariables) {
        const tableBindVar = this.basicNewBindVariable(null, '', '?')
        ...
        return `SELECT * FROM pragma_table_info(${tableBindVar.getVariableName()}) WHERE name = ${columnBindVar.getVariableName()}`
    }
