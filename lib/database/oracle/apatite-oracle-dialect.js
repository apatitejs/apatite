...
class ApatiteOracleDialect extends ApatiteDialect {
    ...
    buildSQLForTableExistence(tableName, bindVariables) {
        const bindVar = this.basicNewBindVariable(null, 1, ':V')
        bindVar.variableValue = tableName
        bindVariables.push(bindVar)
        return `SELECT * FROM USER_TABLES WHERE TABLE_NAME = ${bindVar.getVariableName()}`
    }

    buildSQLForTableColumnExistence(tableName, columnName, bindVariables) {
        const tableBindVar = this.basicNewBindVariable(null, 1, ':V')
        tableBindVar.variableValue = tableName
        bindVariables.push(tableBindVar)
        const columnBindVar = this.basicNewBindVariable(null, 2, ':V')
        columnBindVar.variableValue = columnName
        bindVariables.push(columnBindVar)
        return `SELECT * FROM USER_TAB_COLUMNS WHERE TABLE_NAME = ${tableBindVar.getVariableName()} AND COLUMN_NAME = ${columnBindVar.getVariableName()}`
    }
